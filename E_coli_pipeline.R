...
for (f in c(1:length(base_list))){
  ...
  db$PP1 = unlist(rm_between(db$PP, '(', ')', extract=TRUE))
  db$Tile = db$PP1
  num.row = numrow(db)
...
for (f in c(1:length(phase.percent.list))){
  ...
  if (length(unique(db10$PP1)) < 15) {
    f.size = 15} else if (length(unique(db10$PP1)) %in% c(15, 16)) {
      f.size = 14} else if (length(unique(db10$PP1)) > 16) {
        f.size = 11}
...
for (e in experiment) {
  x = list.files(path =  paste0(file_path1,  e),
                 pattern = '_PF.tsv$', full.names = TRUE, recursive = TRUE)
  PF_list = c(PF_list, x)
}

para_name = unlist(rm_between(PF_list, '_Inc/', '/phase', extract=TRUE))
file_name = unlist(rm_between(PF_list, 'Bioinformatics_analysis/', '/', extract=TRUE))
...
for (f in c(1:length(PF_list))){
  ...
  db$Tile = str_extract(db$PP ,"\\(\\w")
  db$Tile = str_extract(db$Tile ,"\\w")
  db$Tile = gsub('t', 'top', db$Tile)
  db$Tile = gsub('b', 'bottom', db$Tile)
  db$PP = rm_between(db$PP, '%PF (', ')', extract=TRUE)
  db$PP = unlist(db$PP)
  tilelist = unique(db$Tile)
  
  for (p in  c(1:length(tilelist))){
    
    P1 = ggplot(db %>% filter(Tile == tilelist[p]), aes(x = Cycle, y = X.PF, col=PP, group = PP)) + 
      theme_bw() +
      geom_line(size = 1, alpha = 0.5) + 
      geom_point(size= 2, alpha = 0.5) + 
      stat_summary(fun = mean, aes(y = X.PF, col="mean", group=1), 
                   geom = 'line', size=2) +
      stat_summary(fun = mean, aes(y = X.PF, col="mean", group=1), 
                   geom = 'point', size=2.5) + 
      scale_y_continuous(breaks = scales::pretty_breaks()) +
      scale_x_continuous(breaks = scales::pretty_breaks()) +
      labs(x = "Cycle", y = "%PF", 
           title = paste0(file_name[f], ' ', '(', para_name[f], ') ', 'Passing Filter'), col = "PP") +
      theme(plot.title = element_text(hjust = 0.5, size=20)) +
...
para_name1 = unlist(rm_between(bwa_list, '_Inc/', '/match', extract=TRUE))
file_name1 = unlist(rm_between(bwa_list, file_path1, '/', extract=TRUE))

para_name2 = unlist(rm_between(perfect_list, '_Inc/', '/match', extract=TRUE))
...
#Create directory
sapply(dir_list1, function(x) dir.create(x, showWarnings = TRUE, recursive = TRUE, mode = "0777"), USE.NAMES=FALSE)
sapply(dir_list2, function(x) dir.create(x, showWarnings = TRUE, recursive = TRUE, mode = "0777"), USE.NAMES=FALSE)
...
for (f in c(1:length(bwa_list))){
  ...
  for (p in  c(1:length(tilelist))){
    ...
    plot_name = c("REF1_AGCTT", "Total")
    
    for (i in c(1 : length(unique(db$PP)))){
      P2 = ggplot(db %>% filter(PP == unique(db$PP)[i], Pos == tilelist[p]), 
                  aes(x = Cumulative.Length, y = Count, col=Tile, group = Tile)) + 
        theme_bw() +
        geom_line(size = 1, alpha = 0.7) + 
        geom_point(size=1.5, alpha = 0.7) + 
        scale_y_continuous(breaks = scales::pretty_breaks()) +
        ...
        theme(legend.position="bottom") +
        guides(col = guide_legend(nrow = legend.row)) +
        facet_wrap(PP ~., ncol = 1 )
      
      ggsave(filename = paste0(dir_list1[f], file_name1[f], '_BWA_match_' , plot_name[i], ' ', tilelist[p], '.png'), 
             width = 40, height = 20, units = "cm", plot = P2)
    }
  }
}
...
for (f in c(1:length(perfect_list))){
  ...
  for (p in  c(1:length(tilelist))){
    ...
    for (i in c(1 : length(unique(db$PP)))){
      ...
      ggsave(filename = paste0(dir_list2[f], file_name2[f], '_perfect_match_' , plot_name[i], ' ', tilelist[p], '.png'), 
             width = 40, height = 20, units = "cm", plot = P2)
    }
  }
}
...
#experiment_list = sapply(experiment, toString)
para_name = unlist(rm_between(perfect_list, '_Inc/', '/match', extract=TRUE))
...
for (i in c(1:length(perfect_list))){
  db = read.delim(file = perfect_list[i], sep = '\t', header = TRUE, stringsAsFactors = FALSE)
  db1 = db[str_detect(db$PP, "REF1_AGCTT"), ]
  db1 = db1 %>% filter(Count != 0)
  
  if (max(db1$Cumulative.Length) < 25){
    next
  }
  db2 = db1 %>% filter(Cumulative.Length >= 21) %>% group_by(Tile) %>% 
  ...
  P1 = ggplot(db5, aes(x = Tile, y = Value, colour=Type, group = Type)) + 
    ...
    labs(title = paste0(file_name[i], '_(', para_name[i], ')_perfect_read_decrease (PRD)'), x = "Tile", y = "Perfect read decrease rate (%)") +
    theme(plot.title = element_text(hjust = 0.5, size=25)) +
    theme(axis.title.x=element_text(size=25), axis.text.x = element_text(size = 18, angle = 45, hjust = 0.9)) +
    theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0), size=25), 
          axis.text.y = element_text(size = 22)) +
  ...
  ggsave(filename = paste0(dir_list[i], file_name[i], '_', para_name[i], '_perfect_read_decrease_1','.png'), 
         width = 40, height = 20, units = "cm", plot = P1)
  ...
  P2 = ggplot(db2, aes(x = Cumulative.Length, y = percent_decrease, colour=Tile, group = Tile)) + 
    ...
    labs(title = paste0(file_name[i], '_(', para_name[i], ')_perfect_match_decrease_(PRD)'), x = "Cycles", y = "Perfect read decrease rate (%)") +
    theme(plot.title = element_text(hjust = 0.5, size=25)) +
  ...
  if (length(unique(colnames(db6))) < 15) {
    f.size = 15} else if (length(unique(colnames(db6))) >= 15) {
      f.size = 13
    }
  ...
  ggsave(filename = paste0(dir_list[i], file_name[i], '_', para_name[i], '_perfect_read_decrease_2','.png'), 
         width = 40, height = 20, units = "cm", plot = P2a)
...
if (length(file_list) > 0){
  ...
  for (f in c(1: length(file_list2))){
    ...
    tilelist = unlist(rm_between(filepath, '_Inc_', '.bam', extract=TRUE))[-1]
